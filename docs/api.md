...
- 配置文件统一为多任务 `~/.autosync/autosync.conf.yaml`（托盘 / daemon / CLI `sync` / `status` 共用），`--config` 覆盖路径。
...
对指定任务执行单次同步；配置中仅一个任务时可不指定任务名。

```
autosync sync [task] [--config <path>] [--dry-run]
```

| 参数 | 说明 |
|------|------|
| `task` | 任务名；配置中仅一个任务时可省略 |
| `--config <path>` | 多任务配置文件路径（默认 ~/.autosync/autosync.conf.yaml） |
...
```
同步完成: 自动合并 — 同步完成（自动合并成功）
```
退出码 `0`。失败时退出码 `1` 并（按策略）弹系统通知；日志写入 `~/.autosync/logs/autosync.log`。

**请求（dry-run）**

```
$ autosync sync --dry-run
```
...
读取并展示任务同步状态。用宽松加载，允许 repo_dir 暂时不可用；省略 task 时列出全部任务。

```
autosync status [task] [--config <path>]
```
...
写 `~/.config/systemd/user/autosync.service`（`ExecStart` 调 `daemon`）+ `systemctl --user enable --now`。开机即启（无需登录）须 `loginctl enable-linger $USER`（提示见 `scripts/install-linux.sh`）。退出码 `0`。
...
- 托盘菜单：各任务手动同步 / 暂停、开机自启开关、打开配置、退出；同步结果写每任务 state。
...
`ignore` 默认：`*.tmp`、`Thumbs.db`、`desktop.ini`、`.DS_Store`。
